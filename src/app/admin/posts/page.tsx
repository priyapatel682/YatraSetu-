...
export default function AdminPostsPage() {
  ...
  const fetchPosts = async () => {
    try {
      ...
      if (currentUser?.role === "contributor") {
        data = data.filter((p: any) => p.authorId === currentUser.id);
      }
      ...
      const mapped = data.map((p: any) => ({
        ...
        image: p.image,
        date: p.createdAt ? new Date(p.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : "Unknown"
      }));
      setPosts(mapped);
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/posts/${itemToDelete}`, {
        method: "DELETE"
      });
      if (res.ok) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Manage Articles & Posts</h1>
          <p className="text-foreground/60 text-sm mt-1">View, edit, or delete articles and guides.</p>
        </div>
        <Link href="/dashboard/posts/new" className="bg-brand-500 hover:bg-brand-600 text-white px-4 py-2 rounded-xl font-medium transition-colors shadow-sm flex items-center gap-2">
      ...
      <div className="bg-white dark:bg-surface-800 rounded-2xl border border-gray-100 dark:border-white/5 card-shadow overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {loading ? (
            <div className="p-8 text-center text-foreground/50">Loading posts...</div>
          ) : posts.length === 0 ? (
            <div className="p-8 text-center text-foreground/50">No posts found in the database.</div>
          ) : (
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-sm">
                {posts.map((post: any) => (
                  <tr key={post.id} className="border-b border-gray-50 dark:border-white/5 hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4 font-medium text-foreground">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-gray-200 dark:bg-surface-900 shrink-0 overflow-hidden relative border border-gray-100 dark:border-white/10">
                          {post.image ? (
                            <img src={`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}${post.image}`} alt={post.title} className="w-full h-full object-cover" />
                          ) : (
                    ...
                    <td className="px-6 py-4 text-foreground/50">{post.date}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/dashboard/posts/${post.id}/edit`} className="p-2 text-foreground/50 hover:text-brand-600 hover:bg-brand-50 rounded-lg transition-colors" title="Edit">
                          <Edit size={16} />
                        </Link>
                        <button onClick={() => setItemToDelete(post.id)} className="p-2 text-foreground/50 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
        ...
        {posts.length > 0 && (
          <div className="p-4 border-t border-gray-100 dark:border-white/10 flex items-center justify-between text-sm text-foreground/60">
            <span>Showing 1 to {posts.length} of {posts.length} results</span>
